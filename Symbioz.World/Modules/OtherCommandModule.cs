...
namespace Symbioz.World.Modules
{
    class OtherCommandModule
    {
        ...
        [ChatCommand("pos", ServerRoleEnum.Player)]
        public static void PosCommand(string value, WorldClient client)
        {
          client.Character.Reply("Position actuelle : " + client.Character.Map.Position.X + "/" + client.Character.Map.Position.Y);
        }

        [ChatCommand("online", ServerRoleEnum.Player)]
        public static void Online(string value, WorldClient client)
        {
        client.Character.Reply("Il y a actuellement : " + WorldServer.Instance.ClientsCount + " joueur(s) connecté(s)");
        }

        [ChatCommand("leader", ServerRoleEnum.Player)]
        public static void Leader(string value, WorldClient client)
        {
        var lead = WorldServer.Instance.GetOnlineClients().OrderByDescending(x => x.Character.Level).Take(3);
        int numberLead = 1;
        foreach (var target in lead)
        {
            client.Character.Reply($"#{numberLead} - {target.Character.Name} (Niv. {target.Character.Level})");
            numberLead++;
        }
        }
        [ChatCommand("find", ServerRoleEnum.Player)]
        public static void Find(string value, WorldClient client)
        {
         var clientFind = WorldServer.Instance.GetOnlineClients().FirstOrDefault(x => x.Character.Name == value);
         if(clientFind == null)
            {
                client.Character.Reply("Le personnage n'est pas connecté ou n'existe pas");
                return;
            }
         client.Character.Reply($"Joueur trouvé : {clientFind.Character.Name} | Niveau : {clientFind.Character.Level} | Prestige : {clientFind.Character.Record.Prestige} ");
        }

         [ChatCommand("heal", ServerRoleEnum.Player)]
        public static void Heal(string value, WorldClient client)
        {
        client.Character.Record.Stats.LifePoints = client.Character.Record.Stats.MaxLifePoints;
        client.Character.Record.Stats.Energy = client.Character.Record.Stats.MaxEnergyPoints;
        client.Character.RefreshStats();
        client.Character.Reply("Les points de vie et d'énergie ont été restauré");
        }
    }
}