...
{
    /// <summary>
    /// Exemplaire d'un objet appartenant à un personnage joueur.
    /// Hérite de AbstractItem (UId, GId, Position, Quantity, Effects, AppearanceId).
    ///
    /// Contrairement à ItemRecord (template partagé), CharacterItemRecord est une instance unique :
    ///   - UId = identifiant unique de cet exemplaire dans toute la base
    ///   - GId = référence vers l'ItemRecord (template de l'objet)
    ///   - Position = emplacement dans l'inventaire (équipé/non équipé, slot précis)
    ///   - Effects = valeurs des statistiques réellement tirées pour cet exemplaire
    ///
    /// [Resettable] = la table peut être vidée lors d'un reset serveur.
    /// CloneWithoutUID() crée une copie avec un nouveau UId (pour séparer des stacks).
    /// </summary>
    [Table("CharactersItems"), Resettable]
    public class CharacterItemRecord : AbstractItem, ITable
    {
        // Liste statique de tous les objets de tous les personnages chargés en mémoire
        public static List<CharacterItemRecord> CharactersItems = new List<CharacterItemRecord>();

        [Update]
        public long CharacterId; // ID du personnage propriétaire de cet objet
