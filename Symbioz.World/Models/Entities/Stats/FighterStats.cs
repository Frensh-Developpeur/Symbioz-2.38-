...
{
    /// <summary>
    /// Extension de Stats pour le combat : ajoute les informations dynamiques propres à un tour de jeu.
    ///
    /// Contrairement à Stats (persistant), FighterStats est créé à l'entrée en combat par copie des stats
    /// du personnage ou du monstre, et détruit à la fin du combat.
    ///
    /// Nouvelles propriétés de combat :
    ///   - CurrentLifePoints / CurrentMaxLifePoints : HP actuels vs HP max (diminués par l'érosion)
    ///   - Erosion : % de HP érodés (réduit CurrentMaxLifePoints de façon permanente dans le combat)
    ///   - ShieldPoints : points de bouclier absorbant les dégâts avant les HP
    ///   - ApUsed / MpUsed : PA/PM dépensés ce tour (réinitialisés en fin de tour)
    ///   - InvisibilityState : état de visibilité (visible, semi-invisible, invisible)
    ///   - Summoned / SummonerId : indique si c'est une invocation et qui en est le propriétaire
    /// </summary>
    public class FighterStats : Stats
    {
        // Pourcentage de vie restante (CurrentLifePoints / CurrentMaxLifePoints)
        ...
        // Pourcentage de PM restants (PM actuels / PM maximum)
        ...
        // Coefficient multiplicateur de dégâts finaux (modifié par certains effets de sort)
        public double FinalDamageCoefficient = 1;

        // HP perdus depuis le début du combat (CurrentMaxLifePoints - CurrentLifePoints)
        public int LifeLost
        {
            get
            {
        ...
        // HP actuels du combattant (diminue quand il prend des dégâts)
        ...
        // Valeur d'érosion accumulée (réduit CurrentMaxLifePoints de façon permanente dans le combat)
        public int Erosion
        {
            get;
            set;
        ...
        // HP maximum effectifs (MaxLifePoints - érosion accumulée)
        ...
        // Quantité de HP érodés (MaxLifePoints - CurrentMaxLifePoints)
        ...
        // PM dépensés durant le tour en cours (remis à 0 par OnTurnEnded)
        public short MpUsed
        {
            get;
            set;
        ...
        // PA dépensés durant le tour en cours (remis à 0 par OnTurnEnded)
        public short ApUsed
        {
            get;
            set;
        ...
        // État de visibilité du combattant (VISIBLE, TRANSPARENT, INVISIBLE)
        ...
        // Points de bouclier absorbant les dégâts avant les HP (ajoutés par ArmorBuff/ShieldBuff)
        ...
        // Indique si ce combattant est une invocation (invocations passent automatiquement leur tour)
        ...
        // Id du Fighter qui a invoqué ce combattant (0 si non invoqué)
        ...
        // Restaure les PA/PM dépensés en fin de tour (remet le Context à 0 et réinitialise les compteurs)
        ...
        // Dépense des PM : décrémente le Context et incrémente le compteur MpUsed
        ...
        // Dépense des PA : décrémente le Context et incrémente le compteur ApUsed
        ...
        // Ajoute des points de bouclier (peut s'accumuler via plusieurs sorts de protection)
        public void AddShield(short amount)
        {
            ShieldPoints += amount;
        }

        // Retire des points de bouclier (minimum 0, le bouclier ne peut pas devenir négatif)
        public void RemoveShield(short amount)
        {
            ShieldPoints = ShieldPoints - amount < 0 ? 0 : ShieldPoints - amount;
        }

        // Fixe directement la valeur du bouclier (minimum 0)
        ...
        // Marque ce combattant comme invocation et applique le bonus de stats selon le niveau de l'invocateur.
        // Si summonStats = true, toutes les caractéristiques sont recalculées via GetSummonedCharacteristicDelta.
        public void InitializeSummon(Fighter owner, bool summonStats)
        {
            ...
            if (summonStats)
            {
                // Chaque caractéristique est mise à l'échelle selon la formule d'invocation
        ...
        // Initialise une bombe Roublard : marque comme invocation et calcule ses HP via GetBombLife.
        ...
        // Sérialise les stats minimales pour la synchronisation client en combat.
        // GameFightMinimalStats est envoyé après chaque action pour mettre à jour les barres de vie/PA/PM.
