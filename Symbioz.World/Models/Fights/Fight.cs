...
{
    /// <summary>
    /// Classe abstraite représentant un combat dans le jeu.
    /// Gère toute la logique d'un combat : phases (placement → combat → fin),
    /// gestion des tours, des équipes, des marques (glyphes, pièges, portails),
    /// des invocations, et de la fin de combat.
    ///
    /// Les types concrets de combat héritent de cette classe :
    /// FightPvM, FightDual, FightAgression, FightArena, etc.
    /// </summary>
    public abstract class Fight
    {
        ...
        /// <summary>
        /// Durée d'un tour de combat en secondes (30 secondes par défaut).
        /// </summary>
        ...
        /// <summary>
        /// Constructeur : initialise un combat avec ses deux équipes et la map concernée.
        /// Chaque combat reçoit un identifiant unique fourni par FightProvider.
        /// </summary>
        public Fight(MapRecord map, FightTeam blueTeam, FightTeam redTeam, short cellId)
        {
            ...
            // Chaque équipe doit connaître son combat parent
        ...
        /// <summary>
        /// Démarre la phase de placement : affiche les épées sur la map (si applicable)
        /// et lance le timer de préparation. Quand le délai expire, le combat commence automatiquement.
        /// </summary>
        ...
        /// <summary>
        /// Vérifie si tous les combattants des deux équipes sont prêts.
        /// Si oui, lance immédiatement le combat sans attendre le timer.
        /// </summary>
        ...
        /// <summary>
        /// Lance réellement le combat : arrête le timer de placement, met à jour les positions,
        /// retire les épées de la map, ordonne la timeline selon l'initiative, et démarre le premier tour.
        /// </summary>
        ...
        /// <summary>
        /// Démarre un nouveau tour de combat (uniquement si le combat est commencé et non terminé).
        /// </summary>
        ...
        /// <summary>
        /// Termine le tour du combattant actuel :
        /// - arrête le timer de tour
        /// - annule le synchronisateur précédent si existant
        /// - déclenche OnTurnStopped() puis attend la confirmation des clients (Synchronizer)
        ///   avant de vraiment passer au tour suivant (PassTurn).
        /// </summary>
        public void StopTurn()
        {
            if (!Ended)
            {
                ...
                {
                    this.OnTurnStopped();
                    // Demande aux clients de confirmer qu'ils ont bien reçu la fin du tour
        ...
        #endregion
        /// <summary>
        /// Termine le combat : détermine les gagnants, génère et applique les résultats
        /// (XP, kamas, drops), envoie le message de fin de combat aux joueurs,
        /// puis les renvoie sur la map. Libère ensuite les ressources (Dispose).
        /// </summary>
        ...
        /// <summary>
        /// Vérifie si des combattants ont 0 PV ou moins et les fait mourir.
        /// Si le combattant qui vient de mourir était en train de jouer son tour,
        /// son tour est immédiatement passé.
        /// </summary>
        public void CheckDeads()
        {
            // Récupère tous les combattants considérés vivants mais sans PV
            ...
            if (fighters.Count > 0)
            {
                ...
                foreach (var fighter in fighters)
                {
                    // Si c'est le combattant actif qui meurt, on mémorise pour passer son tour ensuite
                ...
                // Si le joueur actif est mort, son tour s'arrête automatiquement
                if (playingFighter != null)// && !Ended)
                    playingFighter.PassTurn();
            }
        }
        /// <summary>
        /// Vérifie si le combat doit se terminer (une équipe n'a plus de survivants).
        /// Si oui, déclenche la fin via un Synchronizer pour s'assurer que tous les clients
        /// sont synchronisés avant d'envoyer le message de fin de combat.
        /// Retourne true si le combat est terminé (ou en cours de fin).
        /// </summary>
        ...
        /// <summary>
        /// Vérifie s'il existe une ligne de vue entre deux cellules.
        /// Utilise un algorithme de tracé de ligne (LineSet) et vérifie pour chaque cellule
        /// intermédiaire si elle bloque la vue (mur ou combattant selon le paramètre throughEntities).
        /// Retourne true si la cellule "to" est visible depuis "from".
        /// </summary>
        public bool CanBeSeen(short from, short to, bool throughEntities = false)
        {
            ...
            // Si on ne passe pas à travers les entités, on collecte les cellules bloquantes
            if (!throughEntities)
            ...
            // Parcourt toutes les cellules entre from et to (sauf la première = position du lanceur)
            var list = line.EnumerateValidPoints().Skip(1);
            // Retourne false si une cellule bloque (pas de ligne de vue OU combattant bloquant)
