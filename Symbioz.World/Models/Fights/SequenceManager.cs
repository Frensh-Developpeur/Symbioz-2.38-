...
{
    /// <summary>
    /// Gère les séquences de combat : des blocs d'actions groupées envoyées aux clients.
    /// Une séquence délimite le début et la fin d'un groupe d'actions (sort lancé, mort d'un
    /// personnage, début/fin de tour...) pour que le client les anime correctement.
    /// Une même séquence ne peut pas être démarrée deux fois simultanément.
    /// </summary>
    public class SequenceManager
    {
        ...
        /// <summary>
        /// Démarre une séquence du type donné. Si une séquence de ce type est déjà active,
        /// ne fait rien et retourne false. Sinon, envoie le message de début aux clients.
        /// Retourne true si la séquence a bien été démarrée.
        /// </summary>
        public bool StartSequence(SequenceTypeEnum sequencetype)
        {
            // Une seule séquence de chaque type à la fois
    ...
    /// <summary>
    /// Représente une séquence individuelle (un groupe d'actions groupées).
    /// Envoie SequenceStartMessage au début et SequenceEndMessage à la fin
    /// pour que les clients sachent quand animer les actions.
    /// </summary>
