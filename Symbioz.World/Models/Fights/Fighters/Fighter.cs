...
namespace Symbioz.World.Models.Fights.Fighters
{
    public abstract class Fighter
    {
        ...
        // Déclenche tous les TriggerBuffs du combattant qui correspondent à l'événement donné
        // trigger = l'événement survenu (BEFORE_ATTACKED, AFTER_MOVE, TURN_BEGIN, etc.)
        // token   = données contextuelles (ex: l'objet Damage pour BEFORE_ATTACKED)
        // Retourne true si au moins un buff demande à "consommer" (supprimer) le flux de dégâts
        public bool TriggerBuffs(TriggerType trigger, object token = null)
        {
            bool result = false;
            // On prend un snapshot des buffs pour éviter les problèmes de modification de liste pendant l'itération
            Buff[] source = this.Buffs.ToArray();
            foreach (TriggerBuff current in
                from triggerBuff in source.OfType<TriggerBuff>()
                where (triggerBuff.Trigger & trigger) == trigger  // Filtre : seuls les buffs dont le trigger correspond
                select triggerBuff)
            {
                // On démarre une séquence côté client (animation de déclenchement)
                this.Fight.SequencesManager.StartSequence(SequenceTypeEnum.SEQUENCE_TRIGGERED);

                // Apply() exécute le callback du buff (ex: BeforeAttacked dans TakenDamageMultiply)
                // Si retourne true → le buff est "consommé" (ex: bouclier absorbant)
                if (current.Apply(trigger, token))
                    result = true;

                // Notifie le client que ce buff vient de se déclencher (affiche l'animation de déclenchement).
                // Paramètres : actionId=0, sourceId=lanceur du sort, targetId=combattant qui déclenche, boostUID=ID du buff
                //
                // POURQUOI current.Caster.Id et pas this.Id comme sourceId ?
                //   sourceId = qui a lancé le sort à l'origine (ex: le personnage qui a lancé Friction).
                //   this.Id  = le combattant qui a le buff et qui le déclenche (ex: le monstre attaqué).
                //   Si on met this.Id comme sourceId, le client cherche le nom de CE combattant
                //   comme "lanceur du sort" et affiche "Unknown Fighter" car ce n'est pas lui qui l'a lancé.
                //   current.Caster.Id pointe vers le vrai lanceur → le client affiche son nom correctement.
                Fight.Send(new GameActionFightTriggerEffectMessage(0, current.Caster.Id, this.Id, current.Id));
        ...
        // Point d'entrée principal pour infliger des dégâts à ce combattant
        // damage      = l'objet Damage avec la source, la cible, le type et le montant
        // triggerBuffs = false uniquement pour les dégâts de renvoi (évite la récursion infinie)
        // Retourne les HP effectivement perdus (0 si immunité, absorbé, etc.)
        public short InflictDamages(Damage damage, bool triggerBuffs = true)
        {
            // Événement "quelqu'un tente d'infliger des dégâts" (hook personnalisé)
            if (OnAttemptToInflictEvt != null)
                OnAttemptToInflictEvt(this, damage);

            // Si la cible est invulnérable : on affiche la réduction mais aucun HP perdu
            if (IsInvulnerable)
            {
                OnDamageReduced(damage.Delta);
                return 0;
            }
            // Immunité corps-à-corps (ex: sort ou buff spécifique)
            if (InvulnerableMelee && Abilities.MeleeWith(damage.Source))
            {
                return 0;
            }
            // Immunité à distance
            if (InvulnerableRange && !Abilities.MeleeWith(damage.Source))
            {
                return 0;
            }

            // Événement pré-dégâts (hook personnalisé)
            if (BeforeTakeDamagesEvt != null)
                BeforeTakeDamagesEvt(this, damage);

            // Déclenche les TriggerBuffs BEFORE_ATTACKED (ex: TakenDamageMultiply = Vulnérabilité)
            // C'est ici que damage.Delta peut être modifié (augmenté ou réduit) par des buffs
            // Si un buff retourne true → les dégâts sont annulés (ex: bouclier absorbant total)
            if (triggerBuffs && this.TriggerBuffs(TriggerType.BEFORE_ATTACKED, damage))
                return 0;

            // Soustrait le renvoi de dégâts du combattant (ex: runes de renvoi)
            damage.Delta -= Stats.Reflect.TotalInContext();// les dommages retournées sont ils soustrait a la somme des dommages?

            // Applique les résistances, l'érosion, les réductions et soustrait les HP
            short result = this.LoseLife(damage, triggerBuffs);

            if (triggerBuffs) // prevent StackOverflow.
            {
                // Renvoie une partie des dégâts à la source si le combattant a du Renvoi
        ...
        // Calcule et applique les dégâts finaux sur les HP du combattant
        // Appelé après InflictDamages() une fois que tous les buffs BEFORE_ATTACKED ont été traités
        // Ordre : résistances élémentaires → érosion → réductions → bouclier → HP
        private short LoseLife(Damage damage, bool triggerBuffs = true)
        {
            // 1. Applique les résistances élémentaires (% et fixes selon le type de dégât)
            damage.CalculateDamageResistance();
            // 2. Calcule l'érosion (réduit les HP max de façon permanente, plafonné à 50%)
            ushort erosionDelta = (ushort)CalculateErosionDamage(damage.Delta);
            // 3. Applique les réductions de dégâts (fixes et %) et soustrait l'érosion
            damage.CalculateDamageReduction(erosionDelta);
            // 4. Applique le bonus final (ex: boost de dégâts reçus)
            damage.ApplyFinalBoost();
            // Si après toutes les réductions les dégâts sont nuls ou négatifs → rien ne se passe
            if (damage.Delta <= 0)
                return 0;

            // ── Cas avec bouclier actif ───────────────────────────────────────────────
            if (Stats.ShieldPoints > 0)
            {
                if (Stats.ShieldPoints - damage.Delta <= 0)
                {
                    // Le bouclier est détruit et il reste des dégâts → les dégâts débordent sur les HP
                    ...
                    {
                        // Les dégâts débordants tuent le combattant
                ...
                {
                    // Le bouclier absorbe tous les dégâts → aucun HP perdu
            ...
            {
                // Les dégâts sont fatals : le combattant tombe à 0 HP (mort)
            ...
            {
                // Cas normal : soustrait les HP et l'érosion, notifie le client
                ...
                // Déclenche les buffs AFTER_ATTACKED (ex: contre-attaque, poison réactif...)
                if (triggerBuffs)
