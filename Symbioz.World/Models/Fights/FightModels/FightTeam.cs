...
{
    /// <summary>
    /// Représente une équipe dans un combat (équipe rouge ou bleue).
    /// Gère la liste des combattants, leurs cellules de placement, les options de l'équipe
    /// (rejoindre/spectateur), et des utilitaires pour trouver le combattant le plus faible,
    /// le plus fort, le plus proche, etc.
    /// </summary>
    public class FightTeam
    {
        // Identifiant de l'équipe : 0 = bleu (défenseur), 1 = rouge (attaquant)
        public sbyte Id
        {
            get;
            set;
        ...
        /// <summary>
        /// Ajoute un combattant à l'équipe, l'initialise avec la référence au combat,
        /// et déclenche OnJoined() si le combat n'a pas encore commencé (phase de placement).
        /// </summary>
        public void AddFighter(Fighter fighter)
        {
            ...
            // Pendant la phase de placement, notifie le combattant de son arrivée
            if (!Fight.Started)
                fighter.OnJoined();
        }
        ...
        /// <summary>
        /// Calcule la direction vers laquelle un combattant doit se tourner en phase de placement.
        /// Le combattant se tourne vers l'ennemi le plus proche de lui.
        /// Si aucun ennemi n'est présent, orientation par défaut vers le sud-ouest.
        /// </summary>
        ...
        {
            return new FightTeamLightInformations(Id, Leader != null ? (double)Leader.Id : 0, (sbyte)Side, (sbyte)Type, 0, (sbyte)Fighters.Count, 50);
        }
        /// <summary>
        ...
        {
            return GetFighters(true).OrderByDescending(x => x.GetMPDistanceBetwenn(source)).FirstOrDefault();
        }
