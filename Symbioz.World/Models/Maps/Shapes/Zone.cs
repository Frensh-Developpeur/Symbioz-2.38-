...
{
    /// <summary>
    /// Représente une zone d'effet de sort sur la map isométrique.
    /// Une Zone est définie par un code de forme (ShapeType, char), un rayon et une direction.
    /// Elle délègue le calcul des cellules à une implémentation concrète d'IShape
    /// choisie dynamiquement par InitializeShape() selon le code de forme.
    ///
    /// Codes de forme (convention Ankama) :
    ///   P = cellule unique (Single), C = losange (Lozenge), D/X = croix (Cross),
    ///   L = ligne (Line), T = croix perpendiculaire, V = cône, W/G = carré (Square),
    ///   a = toutes les cellules, A = tout le terrain, I = anneau, etc.
    /// </summary>
    public class Zone : IShape
    {
        private IShape m_shape;      // Implémentation concrète de la forme géométrique
        private char m_shapeType;    // Code de forme (P, C, D, L, T, V...)
        private byte m_radius;       // Rayon de la zone (en cases)
        private DirectionsEnum m_direction; // Direction (pour les zones orientées comme les cônes)
        // Changement du type de forme : reconstruit automatiquement l'IShape correspondant
        ...
        // Instancie l'IShape concret correspondant au code de forme (switch sur le char)
