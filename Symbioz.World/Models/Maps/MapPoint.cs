...
{
    /// <summary>
    /// Représente une cellule de la map dans le système de coordonnées isométrique de Dofus.
    /// La map utilise une grille isométrique (losange) de 14 colonnes × 20 lignes = 560 cellules.
    /// Chaque cellule a :
    ///   - un CellId (0-559) : identifiant linéaire utilisé dans le protocole réseau
    ///   - des coordonnées (X, Y) dans le repère orthogonal interne
    /// Cette classe fournit tous les outils de navigation : distances, directions,
    /// adjacence, ligne de vue, etc.
    /// La grille de référence est initialisée en statique une seule fois au premier accès.
    /// </summary>
    public class MapPoint
    {
        // Dimensions de la grille de map Dofus standard
        public const uint MapWidth = 14u;
        public const uint MapHeight = 20u;
        public const uint MapSize = 560u;   // 14 × 20 × 2 = 560 cellules

        // Vecteurs de direction dans le système de coordonnées orthogonal
        // Chaque direction correspond à un déplacement (X, Y)
        ...
        // Indique si la grille de référence statique a été initialisée
        private static bool m_initialized;
        // Tableau de référence : pour chaque cellId, stocke le MapPoint correspondant
        // Permet une conversion O(1) de cellId → (X, Y)
        ...
        /// <summary>
        /// Calcule le CellId à partir des coordonnées (X, Y).
        /// Formule : (X - Y) * 14 + Y + (X - Y) / 2
        /// </summary>
        ...
        /// <summary>
        /// Calcule les coordonnées (X, Y) à partir du CellId via la grille de référence.
        /// Lance une exception si le CellId est hors des bornes (0-559).
        /// </summary>
        ...
        /// <summary>
        /// Distance euclidienne (diagonale) entre deux points (formule de Pythagore).
        /// Moins précise pour les déplacements en grille mais utilisée pour certains calculs.
        /// </summary>
        ...
        /// <summary>
        /// Distance en "cases" entre deux cellules (distance de Manhattan = |ΔX| + |ΔY|).
        /// C'est la distance réellement parcourue en PM dans le jeu.
        /// </summary>
        ...
        /// <summary>
        /// Initialise la grille de référence statique (appelée une seule fois).
        /// Pré-calcule les coordonnées (X, Y) pour chacun des 560 CellId possibles.
        /// La grille Dofus est parcourue ligne par ligne (alternant lignes paires et impaires)
        /// pour remplir le tableau OrthogonalGridReference.
        /// </summary>
        private static void InitializeStaticGrid()
        {
            ...
            while ((long)num4 < 20L)
            {
                // Première demi-ligne (14 cellules)
                int num5 = 0;
                while ((long)num5 < 14L)
                {
                ...
                num++;
                // Deuxième demi-ligne (14 cellules, décalée)
                num5 = 0;
                while ((long)num5 < 14L)
                {
                    MapPoint.OrthogonalGridReference[num3++] = new MapPoint(num + num5, num2 + num5);
