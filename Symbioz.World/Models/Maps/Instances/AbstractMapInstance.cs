...
{
    /// <summary>
    /// Instance vivante d'une map : contient toutes les entités présentes sur la map
    /// (personnages, groupes de monstres, PNJ, portails, objets droppés, combats en cours).
    ///
    /// Chaque MapRecord possède une AbstractMapInstance qui gère :
    ///   - La liste des entités (ajout/suppression avec notification client)
    ///   - La liste des combats en cours sur la map
    ///   - Les éléments interactifs (arbres, minerais, zaaps...)
    ///   - Le timer de spawn de monstres (pausé quand aucun joueur n'est présent)
    ///
    /// MapInstance hérite de cette classe abstraite pour la mise en œuvre concrète.
    /// </summary>
    public abstract class AbstractMapInstance
    {
        // Nombre de groupes de monstres actuellement sur la map
        ...
        // Nombre de joueurs actuellement sur la map
        ...
        // Toutes les entités présentes sur la map (joueurs, monstres, PNJ, portails...)
        private List<Entity> m_entities = new List<Entity>();

        // Combats en cours sur cette map
        private List<Fight> m_fights = new List<Fight>();

        // Éléments interactifs actifs (arbres, minerais, zaaps...) sur cette map
        private List<MapInteractiveElement> m_interactiveElements = new List<MapInteractiveElement>();

        // Objets droppés sur le sol (loot après combat)
        private List<DropItem> m_droppedItems = new List<DropItem>();

        // Générateur d'IDs négatifs pour les entités non-joueurs (monstres, PNJ, portails)
        private ReversedUniqueIdProvider m_npIdPopper = new ReversedUniqueIdProvider(0);

        // Générateur d'IDs pour les objets droppés
        ...
        // Appelé après chaque ajout ou retrait d'entité :
        // - Pause le spawn de monstres s'il n'y a plus de joueurs sur la map (économie de ressources)
        // - Reprend le spawn et ajuste l'intervalle selon le nombre de groupes déjà présents
        private void OnEntitiesUpdated()
        {
            if (Record.AbleToSpawn)
            {
                ...
                // Plus il y a de groupes de monstres, plus l'intervalle est long (évite la surpopulation)
