...
{
    /// <summary>
    /// Gestionnaire des portails du Roublard (Xélor).
    /// Détermine la paire de portails actifs (entrée → sortie) pour calculer la téléportation.
    /// GetPortalsTuple retourne le portail d'entrée (cellId) et le portail de sortie (le plus éloigné
    /// dans la chaîne de portails du même joueur).
    /// L'algorithme utilise la distance Manhattan entre portails pour chaîner les téléportations.
    /// GetCasesCount calcule le nombre de cases traversées (utile pour le placement du Xélor).
    /// </summary>
    public class PortalProvider : Singleton<PortalProvider>
    {
        // Sorts qui ne peuvent pas traverser les portails (ex: sort de glyphe de Xélor)
        public ushort[] SPELLS_NOPORTAL = new ushort[]
        {
            5381
        };

        // Constantes de résultat pour l'algorithme de comparaison d'angles
        private const int SAME = 0;             // Même direction

        private const int OPPOSITE = 1;         // Direction opposée

        private const int TRIGONOMETRIC = 2;    // Sens trigonométrique (anti-horaire)

        private const int COUNTER_TRIGONOMETRIC = 3; // Sens horaire

        // Distance maximale (en cases) pour rechercher un portail voisin
        private const int BEST_DIST = 63;

        // Retourne la paire (portail d'entrée, portail de sortie) pour la téléportation
