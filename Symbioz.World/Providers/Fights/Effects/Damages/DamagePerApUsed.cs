...
namespace Symbioz.World.Providers.Fights.Effects.Damages
{
    ...
    /// <summary>
    /// Inflige des dégâts élémentaires proportionnels aux PA utilisés par la cible en fin de tour.
    /// DiceMin/DiceMax × PA utilisés = fourchette du jet de dégâts.
    /// N'inflige rien si jetMin >= jetMax (évite un jet invalide).
    /// Même principe que DamagePerMpUsed mais basé sur les PA au lieu des PM.
    /// </summary>
    public class DamagePerApUsed : SpellEffectHandler
    {
        ...
        {
            // Calcule la fourchette selon les PA utilisés ce tour
            short jetMin = (short)(Effect.DiceMin * buff.Target.Stats.ApUsed);
            short jetMax = (short)(Effect.DiceMax * buff.Target.Stats.ApUsed);

            // N'inflige rien si la fourchette est invalide (aucun PA utilisé ou DiceMin >= DiceMax)
            if (jetMin < jetMax)
            {
