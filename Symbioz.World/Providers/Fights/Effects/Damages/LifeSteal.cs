...
{
    /// <summary>
    /// Vol de vie élémentaire : inflige des dégâts ET soigne le lanceur de la moitié des dégâts infligés.
    /// Pour les éléments non-neutres, le jet est calculé normalement (résistances appliquées).
    /// Pour Effect_StealHPFix (neutre fixe), la valeur est directe sans calcul de jet élémentaire.
    /// Le soin = dégâts effectivement infligés / 2 (après résistances, pas avant).
    /// </summary>
    ...
    {
        // L'élément des dégâts, déterminé à la création depuis l'EffectsEnum
        ...
        {
            // Mappe l'ID d'effet vers l'élément
        ...
        public override bool Apply(Fighter[] targets)
        {
            ...
            {
                // Vol de vie élémentaire : calcule le jet avec résistances
                Jet jet = FormulasProvider.Instance.EvaluateJet(Source, ElementType, Effect, SpellLevel.SpellId);

                foreach (var target in targets)
                {
                    // Inflige les dégâts et soigne le lanceur de la moitié des dégâts réels
            ...
            {
                // Vol de vie fixe (neutre, sans jet) : valeur directe de DiceMin
                foreach (var target in targets)
                {
