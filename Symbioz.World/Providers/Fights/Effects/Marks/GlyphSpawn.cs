...
{
    /// <summary>
    /// Crée un glyphe (zone permanente) sur le terrain.
    /// Effect_Glyph     → se déclenche au DÉBUT du tour d'un combattant qui le traverse (ON_TURN_STARTED).
    /// Effect_Glyph_402 → se déclenche à la FIN du tour (ON_TURN_ENDED).
    /// La couleur et la zone sont définies dans l'effet (Value = couleur ARGB, ShapeType/Radius = zone).
    /// </summary>
    ...
    public class GlyphSpawn : SpellEffectHandler
    {
        ...
        // Détermine quand le glyphe se déclenche selon l'ID de l'effet
