...
namespace Symbioz.World.Providers.Fights.Effects.Buffs
{
    ...
    /// <summary>
    /// Handler pour les buffs de statistiques temporaires en combat.
    /// Gère une très large gamme d'effets qui augmentent temporairement une statistique du fighter :
    /// force, agilité, intelligence, chance, sagesse, portée, dommages bonus, soins, PM, etc.
    /// L'effet est seulement appliqué si Duration > 0 (sinon c'est un effet permanent sans sens).
    /// La valeur est tirée aléatoirement entre DiceMin et DiceMax via RandomizeMinMax().
    /// </summary>
    public class StatsBuff : SpellEffectHandler
    {
        ...
        // Applique le buff de stats sur chaque cible (uniquement pour les effets avec durée)
        public override bool Apply(Fighter[] targets)
        {
            foreach (Fighter current in targets)
            {
                ...
                {
                    // RandomizeMinMax() tire une valeur entre DiceMin et DiceMax de l'effet
        ...
        // Fait correspondre l'EffectsEnum à la Characteristic concernée dans les stats du fighter
