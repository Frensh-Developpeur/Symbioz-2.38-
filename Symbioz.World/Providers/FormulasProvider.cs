...
{
    /// <summary>
    /// Fournit toutes les formules de calcul du jeu : dégâts, soins, initiative,
    /// expérience, drop, coût des zaaps, poids maximum, etc.
    /// Toutes les formules sont centralisées ici pour être facilement modifiables.
    /// Fonctionne en singleton (une seule instance partagée dans tout le serveur).
    /// </summary>
    public class FormulasProvider : Singleton<FormulasProvider>
    {
        // Écart maximum de niveau entre le résultat à crafter et le niveau du métier
        // au-delà duquel aucune XP de métier n'est donnée
        ...
        /// <summary>
        /// Calcule le jet de dégâts d'un effet de sort.
        /// Lit les valeurs min/max de l'effet et délègue au calcul détaillé.
        /// </summary>
        ...
        /// <summary>
        /// Calcule le jet de dégâts d'un sort avec toutes les statistiques du lanceur.
        /// Si jetMax == 0 ou jetMax <= jetMin : le sort a une valeur fixe (pas de plage).
        /// Sinon : tire aléatoirement une valeur entre deltaMin et deltaMax.
        /// Le boost (amélioration du sort) est ajouté aux deux bornes avant calcul.
        /// </summary>
        public Jet EvaluateJet(Fighter source, EffectElementType elementType, short jetMin, short jetMax, short boost, bool useWeapon)
        {
            ...
            {
                // Sort à valeur fixe : pas d'aléatoire
                jetMin += boost;
            ...
            {
                // Sort avec plage de dégâts : tire aléatoirement entre min et max
                jetMin += boost;
        ...
        /// <summary>
        /// Calcule les dégâts finaux d'un jet en appliquant les statistiques élémentaires du combattant.
        /// Formule générale : jet * (100 + caractéristique_élémentaire + bonus%) / 100 + bonus_fixe
        /// Les dégâts ne peuvent pas être inférieurs à la valeur brute du jet (plancher = jet).
        /// </summary>
        private static short GetJetDelta(Fighter fighter, short jet, EffectElementType elementType, bool useWeapon)
        {
            short result;

            // Chaque élément utilise la caractéristique correspondante :
            // Neutre/Terre = Force, Eau = Chance, Air = Agilité, Feu = Intelligence
            switch (elementType)
            {
        ...
        {
            double currentMovementPointsTo = (double)to.Stats.MovementPoints.TotalInContext();

            double maxMovementPointsTo = (double)to.Stats.MovementPoints.Total();
            ...
            double probability = (currentMovementPointsTo / maxMovementPointsTo) * (mpDodgeFrom / mpDodgeTo) * ((double)1 / (double)2);
