...
{
    /// <summary>
    /// Utilitaires de décodage et d'expansion du protocole de déplacement Dofus.
    ///
    /// Dans le protocole réseau, chaque étape de déplacement est encodée dans un short :
    ///   - Bits 0-11  (& 0xFFF = 4095) : identifiant de la cellule
    ///   - Bits 12-15 (>> 12)          : direction (DirectionsEnum, 0-7)
    ///
    /// FightMove sert à re-développer un chemin compressé (seulement les points de rupture)
    /// en chemin complet cellule par cellule avec leur direction.
    /// </summary>
    public class PathParser
    {
        // Extrait l'identifiant de cellule d'un short encodé (retire les 4 bits de direction)
        public static short ReadCell(short cell)
        {
            return (short)(cell & 4095);
        }

        // Extrait la direction d'un entier encodé (les 4 bits de poids fort)
        public static sbyte GetDirection(int result)
        {
            return (sbyte)(result >> 12);
        }

        // Décode une séquence de shorts en dictionnaire {cellId → direction}.
        // Chaque short contient direction (4 bits hauts) + cellId (12 bits bas).
        ...
        // Re-développe un chemin compressé (uniquement les changements de direction)
        // en un chemin complet avec toutes les cellules intermédiaires et leurs directions.
        // Gère les 4 directions orthogonales : SE, SW, NE, NW.
        public static Dictionary<short, DirectionsEnum> FightMove(Dictionary<short, DirectionsEnum> cells)
        {
            ...
            {
                // Ignore les cellules hors des limites de la grille (0-559)
        ...
        // Calcule la coordonnée X isométrique d'une cellule à partir de son identifiant.
        // Utilise la formule issue du protocole Dofus (grille 15 colonnes).
        ...
        // Calcule la coordonnée Y isométrique d'une cellule à partir de son identifiant.
        // La grille Dofus est organisée en rangées de (2*num-1) cellules.
